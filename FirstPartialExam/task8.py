...
def there_is_ball(coord, x,y, balls):
    dx,dy =coord
    ball= (x+dx, y+dy)
    if ball in balls:
        return ball
...
def check_valid_pos(new_x, new_y, obstacles_pos,n, balls):
   return  0 <= new_x < n and 0 <= new_y < n and (new_x, new_y) not in obstacles_pos and (new_x, new_y) not in balls
...
class Balls(Problem):
    ...
    def successor(self, state):
        ...
        for ball in balls:
            ...
            for move in moves:
                ...
                if check_valid_pos(new_x, new_y, self.obstacles, self.n, balls):
                    to_remove = there_is_ball(ball_around[move], x, y, balls)
                    if to_remove is not None:
                        new_balls = balls.copy()
                        new_balls.remove(to_remove)
                        new_balls.remove((x,y))
                        new_balls.append((new_x,new_y))
                        succ[f'{move}: (x={x},y={y})'] = tuple(new_balls)
    ...
    def goal_test(self, state):
        ...
        return len(state) == 1 and ball[0] == self.goal[0] and ball[1] == self.goal[1]
...
if __name__ == "__main__":
    ...
    balls = Balls(tuple(obstacles_pos), n, tuple(balls_pos))
    result = breadth_first_graph_search(balls)
