    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            ...
            for j in range(len(state)):
                if i != j:
                    if len(state[j]) == 0 or int(state[j][-1])>=int(state[i][-1]):
                        new_state = [list(pillar) for pillar in state]
                        moved_block = new_state[i].pop()
                        new_state[j].append(moved_block)
                        succ[f'MOVE TOP BLOCK FROM PILLAR {i+1} TO PILLAR {j+1}'] = tuple(tuple(pillar) for pillar in new_state)
...
if __name__ == "__main__":
    ...
    initial = [tuple(pillar.split(',')) if pillar else () for pillar in initial]
    goal = [tuple(pillar.split(',')) if pillar else () for pillar in goal]

    problem = Blocks(tuple(initial), tuple(goal))
