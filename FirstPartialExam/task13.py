...
class Football(Problem):
    ...
    def check_valid(state, opponents):
        man_pos = state[0]
        man_x, man_y = man_pos
        return (0 <= man_x < 8 and 0 <= man_y < 6 and
                man_pos not in opponents)

    @staticmethod
    def check_ball(state, opponents):
        man_pos, ball_pos = state
        ...
        return (0 <= ball_x < 8 and 0 <= ball_y < 6 and
                ball_pos not in not_allowed)
    ...
    def successor(self, state):
        ...
        for move in moves:
            (dx, dy) = moves[move]
            new_man_x, new_man_y = man_x + dx, man_y + dy
            new_ball_x, new_ball_y = ball_x + dx, ball_y + dy
            new_state = list(state)
            new_state[0] = (new_man_x, new_man_y)
            if self.check_valid(tuple(new_state), self.opponents):
                if ball_x == new_man_x and ball_y == new_man_y:
                    new_state[1] = (new_ball_x, new_ball_y)
                    if self.check_ball(tuple(new_state), self.opponents):
                        successors[f'Turni topka {move}'] = tuple(new_state)
                else:
                    successors[f'Pomesti coveche {move}'] = tuple(new_state)
    ...
    def h(self, node):
        ...
        (x1, y1) = self.goal[0]
        (x2, y2) = self.goal[1]
        return (abs(x1 - ball_x) + abs(y1 - ball_y)) or (abs(x2 - ball_x) + abs(y2 - ball_y))
...
if __name__ == '__main__':
    ...
    footbal = Football(opponents, (man_pos, ball_pos), goal)
    result = astar_search(footbal)
