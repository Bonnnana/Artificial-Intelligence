...
if __name__ == '__main__':
    ...
    # NAIVE BAYES
    train_set_nb = zero[:int(len(zero)*x1)]+one[:int(len(one)*x1)]+two[:int(len(two)*x1)]
    train_x_nb = [row[:-1] for row in train_set_nb]
    train_y_nb = [row[-1] for row in train_set_nb]
    classifier1 = GaussianNB()
    classifier1.fit(train_x_nb, train_y_nb)

    # DECISION TREE
    train_set_dt= zero[int(len(zero) * x1):int(len(zero) * x2)] + one[int(len(one) * x1):int(len(one) * x2)] + two[int(len(two) * x1):int(len(two) * x2)]
    train_x_dt = [row[:-1] for row in train_set_dt]
    train_y_dt = [row[-1] for row in train_set_dt]
    classifier2 = DecisionTreeClassifier(random_state=0)
    classifier2.fit(train_x_dt, train_y_dt)

    #RANDOM FOREST
    train_set_rf = zero[:int(len(zero) * x2)] + one[:int(len(one) * x2)] + two[:int(len(two) * x2)]
    train_x_rf = [row[:-1] for row in train_set_rf]
    train_y_rf = [row[-1] for row in train_set_rf]
    classifier3 = RandomForestClassifier(n_estimators=3, random_state=0)
    classifier3.fit(train_x_rf, train_y_rf)
    ...
    predictions_nb = classifier1.predict(test_x)
    predictions_dt = classifier2.predict(test_x)
    predictions_rf = classifier3.predict(test_x)

    acc = 0
    for pred1, pred2, pred3, true in zip(predictions_nb, predictions_dt, predictions_rf, test_y):
        count = 0
        if true == pred1:
            count += 1
        if true == pred2:
