...
def split_sets(data, x):
    train_set = data[int(len(data) * x):]
    test_set = data[:int(len(data) * x)]
...
if __name__ == '__main__':
    ...
    new_dataset = [row[:-1] + [1] if row[-1] >= 5 else row[:-1] + [0] for row in dataset]

    train_set, test_set, train_x, train_y, test_x, test_y = split_sets(new_dataset, x)

    classifier = DecisionTreeClassifier(criterion='gini', random_state=0)
    classifier.fit(train_x, train_y)
    feature_importances =list(classifier.feature_importances_)
    min_feature = feature_importances.index(min(feature_importances))

    new_dataset_without_min = [[el for i, el in enumerate(row) if i != min_feature] for row in new_dataset]

    train_set_new, test_set_new, train_x_new, train_y_new, test_x_new, test_y_new = split_sets(new_dataset_without_min, x)

    classifier2 = MLPClassifier(hidden_layer_sizes=15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    classifier3 = MLPClassifier(hidden_layer_sizes=15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)

    scaler1 = StandardScaler()
    scaler2 = MinMaxScaler()

    scaler1.fit(train_x_new)
    train_x_std = scaler1.transform(train_x_new)
    test_x_std = scaler1.transform(test_x_new)

    scaler2.fit(train_x_new)
    train_x_minmax = scaler2.transform(train_x_new)
    test_x_minmax = scaler2.transform(test_x_new)

    classifier2.fit(train_x_std, train_y_new)
    classifier3.fit(train_x_minmax, train_y_new)

    predictions1=classifier2.predict(test_x_std)
    predictions2= classifier3.predict(test_x_minmax)

    acc_std = accuracy_score(test_y_new, predictions1)
    acc_minmax = accuracy_score(test_y_new, predictions2)

    print(f"Tocnost so StandardScaler: {acc_std}")
    print(f"Tocnost so MinMaxScaler: {acc_minmax}")
