...
if __name__ == '__main__':
    ...
    classifier = MLPClassifier(hidden_layer_sizes=n, activation='relu', max_iter=20, learning_rate_init=rate)

    classifier.fit(train_x, train_y)
    predictions1 = classifier.predict(train_x)
    accuracy_train = calculate_accuracy(predictions1, train_y)

    predictions2 = classifier.predict(val_x)
    accuracy_val = calculate_accuracy(predictions2, val_y)

    if accuracy_train > accuracy_val * 1.15:
        print("Se sluchuva overfitting")

        new_dataset =[[el for i, el in enumerate(row) if i != col] for row in dataset]

        train_set = new_dataset[:int(len(new_dataset) * 0.8)]
        val_set = new_dataset[int(len(new_dataset) * 0.8):]
