...
def split_sets(x):
    ...
    train_set = ones[:int(x * len(ones))] + zeros[:int(x * len(zeros))]
    test_set = ones[int(x * len(ones)):] + zeros[int(x * len(zeros)):]
...
if __name__ == '__main__':
    ...
    train_set, test_set, train_x, train_y, test_x, test_y = split_sets(X)
    ...
    print(f"Odzivot na kolekcijata so klasifikatori e {odziv}")
